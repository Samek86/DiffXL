...
- 内容ビューの「すべて / 表 / 画像 / セル」チップで、次の差分（F8）と MiniMap 黄マーカーの対象を種類で絞る（本文ストリームは隠さない）
...
- 手動で 1 組対応したあと、残った同名シートは自動で対応する
- 同じ内容ストリーム pair の片側 Text 2 件を `StreamPairIndex` で 1 件にまとめる
- 画像 Match の既定しきい値を類似度 0.55（`ImageRejectDiffRatio` 0.45）に統一
- 表の 2 列行は 1 セル一致だけでは Match せず、行削除＋挿入にする
