...
比較する 2 つの `.xlsx` を選んでください。**Excel のインストールは不要**です。

画面は Excel を再現しません。同じ内容が同じ個数あれば一致、表は枠（または Excel 表）の行、画像は出現順と見た目で合わせます。**数式・マクロ**・ピボット・チャートは見ません。

DiffXL は、セルの値だけでなく **表・図形・埋め込み画像** まで含めて「どこが違うか」を視覚的に示します（OOXML 内容抽出＋自前の内容ビュー）。

| できること | 説明 |
|------------|------|
| **左右分割ビュー** | 2 ブックを並べて同時表示（**操作感のみ** WinMerge 的。Excel 画面の再現ではない） |
...
- アプリ本体は [MIT License](./LICENSE)（Copyright (c) 2026 DiffXL contributors）。
